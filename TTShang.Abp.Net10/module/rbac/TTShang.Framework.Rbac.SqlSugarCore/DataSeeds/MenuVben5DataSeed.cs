...
namespace TTShang.Framework.Rbac.SqlSugarCore.DataSeeds
{
    public class MenuVben5DataSeed : IDataSeedContributor, ITransientDependency
    {
        private ISqlSugarRepository<MenuAggregateRoot> _repository;
        private IGuidGenerator _guidGenerator;
        public MenuVben5DataSeed(ISqlSugarRepository<MenuAggregateRoot> repository, IGuidGenerator guidGenerator)
        {
        ...
        public List<MenuAggregateRoot> GetSeedData()
        {
            ...
            //分配用户
            MenuAggregateRoot roleAuthUser = new MenuAggregateRoot(_guidGenerator.Create(), role.Id)
            {
                MenuName = "分配用户",
                PermissionCode = "system:role:edit",
                MenuType = MenuTypeEnum.Menu,
                Router = "role-auth/user/:roleId",
                IsShow = false,
                IsLink = false,
                IsCache = false,
            ...
            entities.ForEach(m =>
            {
                m.IsDeleted = false;
                m.State = true;
                m.MenuSource = MenuSourceEnum.Vben5;
                m.IsShow = true;
            });
