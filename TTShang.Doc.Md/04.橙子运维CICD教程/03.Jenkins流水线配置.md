...
pipeline {
    ...
    stages {
        stage('git') {
            steps {
                git branch: 'abp', credentialsId: '38c9c74f-8658-4b8d-9ee7-d4c9923ef042', url: 'https://gitee.com/ccnetcore/TTShang.git'
                echo '仓库克隆成功'
            }
        }
        stage('build') {
            steps {
                sh """
                cd TTShang.Abp.Net8/src/TTShang.Abp.Web;
        ...
        stage('zip') {
            steps {
                sh """
                mkdir -p ./publish
                7za a ./publish/publish_01.zip ./TTShang.Abp.Net8/src/TTShang.Abp.Web/bin/Release/net8.0/publish/*;
                """
                echo '压缩包成功'
            }
        }
    ...
    stages {
        ...
        stage('zip') {
            steps {
                sh """
                mkdir -p ./publish
                7za a ./publish/publish_rbac_01.zip ./TTShang.RuoTTShang.Vue3/dist/*;
                """
                echo '压缩包成功'
            }
        }
    ...
    stages {
        ...
        stage('zip') {
            steps {
                sh """
                mkdir -p ./publish
                7za a ./publish/publish_bbs_01.zip ./TTShang.Bbs.Vue3/dist/*;
                """
                echo '压缩包成功'
            }
        }
